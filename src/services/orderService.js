...
async function createOrder(payload) {
  ...
  try {
    ...
    if (!customerId && payload.customer) {
      try {
        ...
        if (error.name === 'SequelizeUniqueConstraintError') {
          throw new Error(
            'CustomerAlreadyExists: Bu e-posta ile kayıtlı müşteri bulunmaktadır. Lütfen mevcut müşteriyi seçerek devam ediniz.'
          );
        }
        throw error;
      }
    ...
    logger.info(
      `Order created: ID ${order.id} for Customer ${customerId} - Total: ${totalAmount}`
    );
