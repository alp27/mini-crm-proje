...
describe('Order Service - Entegrasyon Testleri (Stok & Detaylar)', () => {
  test('Senaryo 1: Guest kullanıcı sipariş verdiğinde Stok düşmeli, Detaylar kaydedilmeli VE listelenmeli', async () => {
    ...
    console.log(
      `✅ Sipariş Oluştu: ID ${order.id}, Tutar: ${order.totalAmount}`
    );
    ...
    console.log(
      `✅ Stok Kontrolü: ${initialStock} -> ${updatedProduct.stock} (Beklenen düştü)`
    );
    ...
    const listedOrders = await orderService.listOrders({
    ...
    expect(fetchedOrder.items[0].product).toHaveProperty(
      'name',
      'Entegrasyon Test Ürünü'
    ); // Product ilişkisi geldi mi?

    console.log(
      `✅ listOrders Kontrolü: Sipariş ve tüm detayları (Customer, Product) başarıyla çekildi.`
    );
  });
  ...
  test('Senaryo 2: Aynı e-posta ile Guest siparişi verilirse hata fırlatmalı', async () => {
    ...
    await expect(orderService.createOrder(orderPayload)).rejects.toThrow(
  ...
  test('Senaryo 3: Stok yetersizse sipariş ve müşteri oluşmamalı (Rollback)', async () => {
    ...
    const checkCustomer = await Customer.findOne({
      where: { email: emailThatShouldNotExist },
    });
    expect(checkCustomer).toBeNull();

    console.log(
      '✅ Başarılı: Hata alındı ve Transaction geri alındı (Müşteri silindi).'
    );
  });
});
