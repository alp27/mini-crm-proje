...
describe('Customer Service Birim Testleri', () => {
  ...
  test('Talep 2.1: İsim ve Soyisim AYNI olsa bile iletişim bilgileri farklıysa kaydedilmeli', async () => {
    ...
    console.log(
      '2. Müşteri oluşturuluyor: Ahmet Yılmaz (Tel: ...2) -> Farklı Kişi'
    );
    ...
    console.log(
      `BAŞARILI: İki farklı "Ahmet Yılmaz" sisteme eklendi. (ID: ${customer1.id} ve ${customer2.id})`
    );
  });
  ...
  test('Talep 2.2: Aynı EMAIL adresiyle ikinci kayıt engellenmeli', async () => {
    ...
    console.log(
      '2. Müşteri (Taklitçi) aynı email ile eklenmeye çalışılıyor...'
    );
    ...
    await expect(
      customerService.createCustomer(duplicatePayload)
  ...
  test('Talep 2.3: Aynı TELEFON numarasıyla ikinci kayıt engellenmeli', async () => {
    ...
    console.log(
      '2. Müşteri aynı telefon numarası ile eklenmeye çalışılıyor...'
    );
    const duplicatePayload = {
      firstName: 'Taklitçi',
