...
async function testOrderAPI() {
    ...
    try {
        ...
        if (customers.length === 0) {
            throw new Error(' ÖNCE MÜŞTERİ EKLE: Veritabanında hiç müşteri yok, sipariş testi yapılamaz.');
        }

        const validCustomerId = customers[0].id; 
        console.log(`  Test için kullanılacak Müşteri ID: ${validCustomerId} (${customers[0].firstName})`);
        ...
        if (createRes.status === 201) {
            ...
            throw new Error(`Sipariş oluşturulamadı: ${JSON.stringify(createdOrder)}`);
        }
        ...
        if (orderDetail.customer && orderDetail.customer.firstName) {
            console.log(`    BAŞARILI: Siparişle birlikte müşteri bilgisi geldi: ${orderDetail.customer.firstName}`);
        } else {
        ...
        if (found) {
            console.log(`    BAŞARILI: Filtreleme çalışıyor, sipariş listede bulundu.`);
        } else {
            console.error('    HATA: Filtreleme çalışmadı veya sipariş bulunamadı.');
        }

        console.log('\n  Test: Siparişi Silme');
        const deleteRes = await fetch(`${BASE_URL}/orders/${orderId}`, { method: 'DELETE' });
